...
protocol MenuManagerDelegate: AnyObject {
    var statusItem: NSStatusItem? { get set } // Allow MenuManager to manage statusItem
    var activeTimers: [(id: UUID, name: String?, startTime: Date, duration: TimeInterval, timer: Timer, reminderId: String?)] { get }
...
class MenuManager: NSObject {
    ...
    @objc private func menuWillOpen(_ notification: Notification) {
        guard let menu = notification.object as? NSMenu, menu == delegate?.statusItem?.menu else { return }
        isMenuOpen = true
  ...
  func updateMenu() {
      ...
      if delegate?.activeTimers.count ?? 0 > 0 {
          ...
          if let activeTimers = delegate?.activeTimers {
              for (index, timerInfo) in activeTimers.enumerated() {
                  ...
                  let formattedTime = delegate?.formatTimeInterval(remainingTime) ?? "00:00"

                  let displayName = timerInfo.name ?? "Timer \(index + 1)"
                  let menuItem = NSMenuItem()
                  menuItem.representedObject = index

                  // Re-integrate HoverView here
                  let customView = HoverView(
                      normalText: "\(displayName): \(formattedTime)",
                      hoverText: "Delete",
                      frame: NSRect(x: 0, y: 0, width: 250, height: 22),
                      index: index,
                      target: self, // Target for HoverView is MenuManager again
                      action: #selector(callCancelTimer(sender:)) // Action for HoverView click
                  )
                  menuItem.view = customView // Set HoverView as the view for the menu item
      ...
      let preferencesMenuItem = NSMenuItem(title: "Preferences", action: #selector(callPreferences), keyEquivalent: ",")
      preferencesMenuItem.target = self
      menu.addItem(preferencesMenuItem)

      let quitMenuItem = NSMenuItem(title: "Quit Dropclock", action: #selector(callQuit), keyEquivalent: "q")
    ...
    // MARK: - Menu Actions Proxy (to call AppDelegate actions)
    ...
    func setupMenu(statusItem: NSStatusItem) {
        delegate?.statusItem = statusItem // Set statusItem via delegate
