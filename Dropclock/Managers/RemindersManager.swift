...
class RemindersManager {
  ...
  func createReminder(title: String, notes: String?, dueDate: Date) throws -> String {
    ...
    // Round up the dueDate to the next full minute
    let roundedDueDate = Calendar.current.date(bySetting: .second, value: 0, of: dueDate.addingTimeInterval(59 - (dueDate.timeIntervalSinceReferenceDate.truncatingRemainder(dividingBy: 60))))
    
    let reminder = EKReminder(eventStore: eventStore)
    reminder.title = title
    reminder.notes = notes
    reminder.calendar = calendar
    reminder.dueDateComponents = Calendar.current.dateComponents([.year, .month, .day, .hour, .minute], from: roundedDueDate!)
