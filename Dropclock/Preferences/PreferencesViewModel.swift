...
class PreferencesViewModel: ObservableObject {
    ...
    @AppStorage("selectedReminderListID") private var selectedReminderListID: String = ""
    ...
    func loadPreferences() {
        if let storedList = RemindersManager.shared.fetchReminderLists().first(where: { $0.calendarIdentifier == selectedReminderListID }) {
  ...
  func savePreferences() {
      ...
      if let selected = selectedList {
          UserDefaults.standard.set(selected.calendarIdentifier, forKey: "SelectedReminderList")
      }
  ...
  func ensureReminderAccess() async {
      ...
      case .notDetermined:
          ...
          // Use Task to ensure it runs on the main thread
          Task { [weak self] in
      ...
      case .denied:
          print("Reminders access denied. Ask the user to enable it in System Settings.")
  ...
  func fetchReminderLists() async {
      // Run the list fetching on the main thread to update UI
  ...
  func updateLoginItem() {
          do {
              ...
              print("Error \(startAtLogin ? "enabling" : "disabling") login item: \(error)")
              startAtLogin.toggle() // Revert toggle if error occurs
          }
      }
}
